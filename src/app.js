...
async function start(){
 ...
 async function handleUpdate(u){if(u.callback_query)return callback(u.callback_query);const m=u.message||u.channel_post;if(m?.from)await saveUser(m.from);if(m?.chat)await saveGroup(m.chat);if(u.message?.text?.startsWith('/'))return command(u.message);if(u.channel_post)return channelPost(u.channel_post);if(m?.reply_to_message)return comment(m);}
 ...
 async function findGiveaway(msg){const r=msg.reply_to_message;if(r){const postId=r.forward_from_message_id||r.message_id;const ch=String(r.forward_from_chat?.id||r.chat?.id||'');const g=await Giveaway.findOne({channelPostId:postId,$or:[{channelId:ch},{discussionChatId:String(msg.chat.id)}]}).sort({createdAt:-1});if(g)return g;}return Giveaway.findOne({status:{$in:['active','completed','picking']}}).sort({createdAt:-1});}
 async function comment(m){if(!['group','supergroup'].includes(m.chat.type))return;const g=await findGiveaway(m);if(g?.status==='active')await recordComment(g,m);}
