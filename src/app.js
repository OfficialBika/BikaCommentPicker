...
async function start(){
 ...
 async function findGiveaway(msg,explicitId){if(explicitId){const byId=await Giveaway.findById(explicitId).catch(()=>null);if(byId)return byId;}const r=msg.reply_to_message;if(r){const postId=r.forward_from_message_id||r.message_id;const ch=String(r.forward_from_chat?.id||r.chat?.id||'');const g=await Giveaway.findOne({channelPostId:postId,$or:[{channelId:ch},{discussionChatId:String(msg.chat.id)}]}).sort({createdAt:-1});if(g)return g;}return null;}
